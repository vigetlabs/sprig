...
describe Sow::DirectiveList do
  ...
  describe "#add_seeds_to_hopper" do
    ...
    before do
      Sow::Directive.stub(:new).with(Post).and_return(directive)

      Sow::Seed::Factory.stub(:new_from_directive).with(directive).and_return(seed_factory)
    end
